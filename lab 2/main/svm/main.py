...
def RBFKernel(x, y, theta):
    return math.exp(-np.dot(np.subtract(x,y),np.subtract(x,y))/(2*pow(theta, 2)))
    
def SigKernel(x, y, k, eta):
    return math.tanh(np.subtract(k*np.dot(x, y),eta))
...
Main()
#PolyKernel(x[i][0:2], x[j][0:2], 3)
#PolyKernel(xstar, support[i][0:2], 3)